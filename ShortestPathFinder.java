import java.util.List;
    ...
    public class ShortestPathFinder {
        ...
        // Hoofdprogramma om het kortste pad te vinden tussen twee kamers
        public static void main(String[] args) {
            Room startRoom = Room.FORUM; // Definieer startkamer
            Room endRoom = Room.H5; // Definieer eindkamer

            // Zoek het kortste pad tussen start- en eindkamer
            List<Room> shortestPath = findShortestPath(startRoom, endRoom);

            // Als er een kortste pad is gevonden, print het pad
            if (shortestPath != null) {
                System.out.println("Shortest path from " + startRoom + " to " + endRoom + ":");
                for (Room room : shortestPath) {
                    System.out.println(room);
                }
            } else {
                System.out.println("No path found from " + startRoom + " to " + endRoom);
            }
        }
    }
